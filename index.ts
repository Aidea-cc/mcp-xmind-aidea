...
interface XMindTopic {
    id: string;
    title: string;
    attributedTitle?: Array<{ text: string }>;
    structureClass?: string;
    children?: {
        attached: XMindTopic[];
        callout?: XMindTopic[];
...
const CreateTopicSchema: z.ZodType<{
    ...
    structureClass?: string;
    topicClass?: 'topic' | 'importantTopic' | 'minorTopic';
    taskStatus?: 'todo' | 'done';
    progress?: number;
    priority?: number;
    startDate?: string;
...
class XMindBuilder {
    ...
    build(sheets: z.infer<typeof CreateXMindArgsSchema>['sheets']): {
        ...
        const contentJson = builtSheets.map(({ rootTopic, sheet }) => {
            ...
            if (hasPlannedTasks) {
                ...
                const existingExtensions = sheetObj.extensions as Array<{ provider: string; content: unknown }>;
    ...
    private buildTopic(input: z.infer<typeof CreateTopicSchema>): XMindTopic {
        ...
        const topic: Record<string, unknown> & XMindTopic = { id, class: nodeClass, title: input.title };
